...
export async function POST(request: Request) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const duplicate = await tx.redemptionRecord.findUnique({ where: { codeId_userId: { codeId: code.id, userId: authz.user.id } } });
      if (duplicate) throw new Error("CODE_USED");
      const user = await tx.user.update({ where: { id: authz.user.id }, data: { points: { increment: code.points } }, select: { points: true } });
      await tx.redemptionCode.update({ where: { id: code.id }, data: { usedCount: { increment: 1 } } });
      await tx.redemptionRecord.create({ data: { codeId: code.id, userId: authz.user.id, points: code.points } });
      await tx.pointTransaction.create({ data: { userId: authz.user.id, type: "RECHARGE", amount: code.points, balanceAfter: user.points, description: `兑换码充值 ${code.code}`, referenceId: code.id } });
