...
export async function POST(_: Request, { params }: Context) {
  ...
  const project = await prisma.novelProject.findFirst({ where: { id, userId: authz.user.id } });
  ...
  const locked = await prisma.novelProject.updateMany({
    where: { id, userId: authz.user.id, status: { in: ["DRAFT", "FAILED"] } },
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      await chargeUsage(authz.user.id, response.model, response.usage, project.id, `生成《${project.title}》大纲`, tx);
    ...
    if (error instanceof Error && (error.message === "INSUFFICIENT_POINTS" || error.message === "ACCOUNT_DISABLED")) {
      await prisma.novelProject.update({ where: { id }, data: { status: "FAILED", errorCode: error.message, errorMessage: error.message === "ACCOUNT_DISABLED" ? "账号已被停用" : "算力点不足" } });
      return billingErrorResponse(error);
    }
